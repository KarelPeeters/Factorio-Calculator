...
class GameData(
    ...
    fun findRecipe(name: String) = recipes.find { it.name == name }
        ?: throw IllegalArgumentException("recipe '$name' not found, similar: " + recipes.map { it.name }
            .suggestions(name))

    fun findModule(name: String) = modules.find { it.name == name }
        ?: throw IllegalArgumentException("module '$name' not found, similar: " + modules.map { it.name }
            .suggestions(name))

    fun findAssembler(name: String) = assemblers.find { it.name == name }
        ?: throw IllegalArgumentException("assembler '$name' not found, similar: " + assemblers.map { it.name }
            .suggestions(name))
}
